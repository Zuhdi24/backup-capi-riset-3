...
//                since the app can't handle current url.
                            String uploadUrl = URLDecoder.decode(locations[0].getValue(), "utf-8");
                            if (uploadUrl.contains(constrainUrlText)) {
                                Log.d("RAHADI", "Replacing URL...");
                                Log.d("RAHADI", "Original URL    : " + uploadUrl);
                                uploadUrl = uploadUrl.replace(constrainUrlText, newUrlText);
                                Log.d("RAHADI", "Becomes         : " + uploadUrl);
                            }
                            Uri uNew = Uri.parse(uploadUrl);
