...
public class FormEntryActivity extends Activity implements AnimationListener,
    ...
    private static final int MENU_NOTE = Menu.FIRST + 4;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        actionBarTitle = getString(R.string.loading_form);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        Drawable save = getResources().getDrawable(R.drawable.ic_save_b);
        Drawable hierarchy = getResources().getDrawable(R.drawable.ic_menu_goto_b);
        Drawable note = getResources().getDrawable(R.drawable.ic_note);
        ...
        CompatibilityUtils.setShowAsAction(
                ...
                MenuItem.SHOW_AS_ACTION_NEVER);
        CompatibilityUtils.setShowAsAction(
                menu.add(0, MENU_NOTE, 0, R.string.catatan)
                        .setIcon(note),
                MenuItem.SHOW_AS_ACTION_IF_ROOM);
        return true;
    }
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        useability = mAdminPreferences.getBoolean(
                AdminPreferencesActivity.KEY_OPEN_NOTE, true);

        menu.findItem(MENU_NOTE).setVisible(useability)
                .setEnabled(useability);
        return true;
    }

    public void savethat(boolean status) {
        String constraint_behavior = PreferenceManager.getDefaultSharedPreferences(getApplicationContext())
                .getString(PreferencesActivity.KEY_CONSTRAINT_BEHAVIOR,
                        PreferencesActivity.CONSTRAINT_BEHAVIOR_DEFAULT);
        Log.d("budisave", "status1");
        if (Collect.getInstance().getFormController().currentPromptIsQuestion()) {
            ...
            if (constraint_behavior.equals(PreferencesActivity.CONSTRAINT_BEHAVIOR_ON_SWIPE)) {
                Log.d("budisave", "status2");
                if (!OriginalsaveAnswersForCurrentScreen(EVALUATE_CONSTRAINTS)) {
                }
                saveDataToDisk(status, isInstanceComplete(false),
                        null, 0);
                Log.d("budisave", "status3.1");
            } else {
                Log.d("budisave", "status4");
                if (saveAnswersForCurrentScreen(DO_NOT_EVALUATE_CONSTRAINTS)) {
                    saveDataToDisk(status, isInstanceComplete(false), null, 0);
                    Log.d("budisave", "status5");
                }
            }
        } else {
            Log.d("budisave", "status6");
            if (saveAnswersForCurrentScreen(DO_NOT_EVALUATE_CONSTRAINTS)) {
                Log.d("budisave", "status7");
                saveDataToDisk(status, isInstanceComplete(false), null, 0);
            }
        }
    }
        ...
        switch (item.getItemId()) {
            ...
            case MENU_HIERARCHY_VIEW:
                ...
                if (Collect.getInstance().getFormController().currentPromptIsQuestion()) {
                    // if constraint behavior says we should validate on swipe, do so
                    if (constraint_behavior.equals(PreferencesActivity.CONSTRAINT_BEHAVIOR_ON_SWIPE)) {
                        OriginalsaveAnswersForCurrentScreen(EVALUATE_CONSTRAINTS);
    ...
    private boolean saveAnswersForCurrentScreen(boolean evaluateConstraints) {
        ...
        if (formController.currentPromptIsQuestion()) {
            LinkedHashMap<FormIndex, IAnswerData> answers = ((ODKView) mCurrentView)
                    .getAnswers();
            try {
                FormController.validasierror constraint = formController.saveAllScreenAnswers(answers, evaluateConstraints);
                if (constraint.ndakbolehdskip.size() != 0) {
                    createConstraintToast(constraint.ndakbolehdskip);
                    //addConstaraintbreaks(constraint);
                    return false;
                } else if (constraint.bolehdskip.size() != 0) {
                    addConstaraintbreaks(constraint.bolehdskip);
                    return false;
                }
            } catch (JavaRosaException e) {
                Log.e(t, e.getMessage(), e);
    ...
    private boolean OriginalsaveAnswersForCurrentScreen(boolean evaluateConstraints) {
        FormController formController = Collect.getInstance()
                .getFormController();
        // only try to save if the current event is a question or a field-list
        // group
        if (formController.currentPromptIsQuestion()) {
            LinkedHashMap<FormIndex, IAnswerData> answers = ((ODKView) mCurrentView)
                    .getAnswers();
            try {
                FormController.validasierror constraint1 = formController.saveAllScreenAnswers(answers, evaluateConstraints);
                if (constraint1.ndakbolehdskip.size() != 0 || constraint1.bolehdskip.size() != 0) {
                    ArrayList<FailedConstraint> sav = new ArrayList<FailedConstraint>();
                    sav = constraint1.ndakbolehdskip;
                    for (FailedConstraint as : constraint1.bolehdskip) {
                        sav.add(as);
                    }
                    ArrayList<DBErrorModel> dem = new ArrayList<DBErrorModel>();
                    for (FailedConstraint constraint : sav) {
                        String aXPathNeo = formController.getXPath(constraint.index);
                        String NullaXPathNeo = formController.getNullXPath(constraint.index);
                        DBErrorModel dem1 = new DBErrorModel();
                        dem1.setJr_Form_Id(formController.getFormTitle());
                        if (JRFromVersion != null) {
                            Log.d("Budi", JRFromVersion);
                            dem1.setJR_Forn_Version(JRFromVersion);
                        }
                        dem1.setXPath(aXPathNeo);
                        dem1.setNullXPath(NullaXPathNeo);
                        dem1.setUUID(formController.getSubmissionMetadata().instanceId);
                        dem1.setInstance_Path(formController.getInstancePath().getAbsolutePath());
                        if (constraint.status == 1) {
                            dem1.setKind(InstanceProviderAPI.REQUIR);
                        } else if (constraint.status == 2) {
                            dem1.setKind(InstanceProviderAPI.Constara);
                        }
                        dem.add(dem1);
                    }
                    addError(dem);
                    createConstraintToast(sav);
                    OriginalsaveAnswersForCurrentScreen(DO_NOT_EVALUATE_CONSTRAINTS);
                    return false;
                }
            } catch (JavaRosaException e) {
                Log.e(t, e.getMessage(), e);
    ...
    private String makeConstraintText(ArrayList<FailedConstraint> Failed) {
        FormController formController = Collect.getInstance()
                .getFormController();
        StringBuilder constraintText = new StringBuilder();
        for (int in = 0; in < Failed.size(); in++) {
            FormIndex index = Failed.get(in).index;
            int saveStatus = Failed.get(in).status;
            String constraintText1 = "";
            Log.d("makeConst", "" + Failed.get(in).status);
            Log.d("makeConst", "" + Failed.get(in).index.getReference());
            switch (saveStatus) {
                case FormEntryController.ANSWER_CONSTRAINT_VIOLATED:
                    constraintText1 = null;
                    Collect.getInstance()
                            .getActivityLogger()
                            .logInstanceAction(this,
                                    "createConstraintToast.ANSWER_CONSTRAINT_VIOLATED",
                                    "show", index);
                    constraintText1 = formController
                            .getQuestionPromptConstraintText(index);
                    if (constraintText1 == null) {
                        constraintText1 = formController.getQuestionPrompt(index)
                                .getSpecialFormQuestionText("constraintMsg");
                        if (constraintText1 == null) {
                            constraintText1 = getString(R.string.invalid_answer_error);
                        }
                    }
                    Log.d("makeConst", constraintText1);
                    constraintText.append(constraintText1);
                    break;
                case FormEntryController.ANSWER_REQUIRED_BUT_EMPTY:
                    ...
                    if (constraintText1 == null) {
                        ...
                        if (constraintText1 == null) {
                            constraintText1 = getString(R.string.required_answer_error);
                        }
                    }
                    Log.d("makeConst", constraintText1);
                    constraintText.append(constraintText1);
                    break;
                default:
                    constraintText.append("Error not identified");
            }
            if (in != (Failed.size() - 1)) {
        ...
        return constraintText.toString();
    }

    private void addConstaraintbreaks(ArrayList<FailedConstraint> Failed) {

        final FormController formController = Collect.getInstance()
                .getFormController();
        String constraintText = makeConstraintText(Failed);
        ArrayList<DBErrorModel> dem = new ArrayList<DBErrorModel>();
        for (FailedConstraint Fc : Failed) {
            FormIndex index = Fc.index;
            int saveStatus = Fc.status;
            String XPathNeo = formController.getXPath(index);
            String nullXPathNeo = formController.getNullXPath(index);
            String qst = formController.getQuestionPrompt(index).getLongText();
            DBErrorModel dem1 = new DBErrorModel();
            dem1.setJr_Form_Id(formController.getFormTitle());
            if (JRFromVersion != null) {
                Log.d("Budi", JRFromVersion);
            ...
            Log.d("budihirar di enty", XPathNeo);
            dem1.setXPath(XPathNeo);
            dem1.setNullXPath(nullXPathNeo);
            dem1.setUUID(formController.getSubmissionMetadata().instanceId);
            dem1.setInstance_Path(formController.getInstancePath().getAbsolutePath());
            dem1.setCheck(InstanceProviderAPI.YESs);
            if (Fc.status == 1) {
                dem1.setKind(InstanceProviderAPI.REQUIR);
            } else if (Fc.status == 2) {
                dem1.setKind(InstanceProviderAPI.Constara);
        ...
        final ArrayList<DBErrorModel> as = dem;
        mAlertDialog = new AlertDialog.Builder(this)
                .setTitle("validasi salah")
                .setMessage(constraintText + "\nskip validasi?")
                .setPositiveButton("Ya", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        addError(as);
                        showNextViewWithoutConstraint();
                    }
                })
    ...
    private void addError(ArrayList<DBErrorModel> aa) {
        FormController fm = Collect.getInstance().getFormController();
        for (DBErrorModel a : aa) {
            fm.addError(a);
        }
    }
        ...
        switch (event) {
            case FormEntryController.EVENT_BEGINNING_OF_FORM:
                ...
                if (image == null) {
                    ...
                    startView.findViewById(R.id.form_start_bling)
                ...
                ImageView ia = ((ImageView) startView
                        .findViewById(R.id.image_usap));
//				TextView ta = ((TextView) startView.findViewById(R.id.text_usap));
//				TextView tb = ((TextView) startView.findViewById(R.id.text_usap_2));
                TextView d = ((TextView) startView.findViewById(R.id.description));
                Typeface QUICKSAND = ResourcesCompat.getFont(this, R.font.quicksand);
                d.setTypeface(QUICKSAND);
            ...
            case FormEntryController.EVENT_END_OF_FORM:
                ...
                String StatusIsian = null;

                formController.validates(true);
                View endView = null;
                StringBuilder textakhir = new StringBuilder();
                if (formController.getError().size() == 0) {
                    textakhir.append("Kamu berada di akhir dari kuesioner\n" + formController.getFormTitle() + ".");
                } else {
                    textakhir.append("Kamu berada di akhir dari kuesioner\n" + formController.getFormTitle() + ".\n Ditemukan " + formController.getError().size() + " error!!!");
                }
                Cursor ress = null;
                String ressnim = null;
                String[] pr = {InstanceColumns.NIM};
                String whe = InstanceColumns.INSTANCE_FILE_PATH + " = ? ";
                String[] wheargss = {formController.getInstancePath().getAbsolutePath(),};
                try {
                    ress = Collect.getInstance().getContentResolver().query(InstanceColumns.CONTENT_URI,
                            pr, whe, wheargss, null);
                    if (ress.moveToFirst()) {
                        do {
                            ressnim = ress.getString(ress.getColumnIndex(InstanceColumns.NIM));
                ...
                String[] projs = {InstanceColumns.DISPLAY_NAME};
                String wheres = InstanceColumns.INSTANCE_FILE_PATH + " = ? AND " + InstanceColumns.CAN_EDIT_FILENAMA + " =? ";
                String[] whereargss = {formController.getInstancePath().getAbsolutePath(),
                        InstanceProviderAPI.NOs};
                String idd = null;
                try {
                    resultss = Collect.getInstance().getContentResolver().query(InstanceColumns.CONTENT_URI,
                            projs, wheres, whereargss, null);
                    if (resultss.moveToFirst()) {
                        do {
                            idd = resultss.getString(resultss.getColumnIndex(InstanceColumns.DISPLAY_NAME));
                            Log.e("DISPLAY NAME", idd);
                ...
                if (ressnim == null || idd == null || idJabatan.equalsIgnoreCase("0")) {
                    formController.setMetadataTambahan();
                    endView = View.inflate(this, R.layout.form_entry_end, null);
                    if (idd == null) {
                        idd = formController.getNama();
                    }
                    if (idd == null) {
                        int id = 1;
                        Cursor resultss2 = null;
                        String[] projs2 = {InstanceColumns._ID};
                        try {
                            resultss2 = Collect.getInstance().getContentResolver().query(InstanceColumns.CONTENT_URI,
                                    projs2, null, null, InstanceColumns._ID + " DESC LIMIT 1");
                            if (resultss2.moveToFirst()) {
                                do {
                                    if (resultss2.getString(resultss2.getColumnIndex(InstanceColumns._ID)) != null) {
                                        id = Integer.valueOf(resultss2.getString(resultss2.getColumnIndex(InstanceColumns._ID)));
                                        Log.d("nameL", "" + id);
                                        id += 1;
                                    }
                                } while (resultss.moveToNext());
                        ...
                        idd = formController.getFormTitle() + "_" + nim + "_" + id;
                        Log.d("IDD NULL", idd);
                    }

                    ((TextView) endView.findViewById(R.id.description))
                            .setText(textakhir.toString());
                    if (formController.getError().size() != 0) {
                        ((TextView) endView.findViewById(R.id.description))
                                .setTextColor(getResources().getColor(R.color.pkl_error));
                    }
                    ...
                    final TextView saveAs = (TextView) endView
                    ...
                    endView.findViewById(R.id.check_button)
                            .setOnClickListener(new OnClickListener() {
                                @Override
                                public void onClick(View v) {
                                    Collect.getInstance()
                    ...
                    endView = View.inflate(this, R.layout.form_entry_end_kortim, null);
                    Cursor resultss1 = null;
                    String[] projs1 = {InstanceColumns.STATUS_ISIAN, InstanceColumns.STATUS};
                    String wheres1 = InstanceColumns.INSTANCE_FILE_PATH + " =? ";
                    String[] whereargss1 = {formController.getInstancePath().getAbsolutePath(),};
                    try {
                        resultss1 = Collect.getInstance().getContentResolver().query(InstanceColumns.CONTENT_URI,
                                projs1, wheres1, whereargss1, null);
                        if (resultss1.moveToFirst()) {
                            do {
                                StatusIsian = resultss1.getString(resultss1.getColumnIndex(InstanceColumns.STATUS_ISIAN));
                                //Status=resultss1.getString(resultss1.getColumnIndex(InstanceColumns.STATUS));
                            } while (resultss1.moveToNext());
                        }
                    } catch (Exception ex) {
                        Log.d(t, "name " + ex);
                    } finally {
                    ...
                    final RadioGroup stat = (RadioGroup) endView.findViewById(R.id.grup_status);
                    stat.check(R.id.cleared);
                    final RadioGroup statI = (RadioGroup) endView.findViewById(R.id.grup_status_error);
                    if (StatusIsian.equals(InstanceProviderAPI.STATUS_CLEAR)) {
                        statI.check(R.id.clear);
                    } else {
                        statI.check(R.id.unclear);
                    }
                            ...
                            .setOnClickListener(new OnClickListener() {
                                ...
                                public void onClick(View v) {
                                    ...
                                    if (saveAs.getText().length() < 1) {
                                        ...
                                        if (stat.getCheckedRadioButtonId() == 0 || statI.getCheckedRadioButtonId() == 0) {
                                            Toast.makeText(FormEntryActivity.this,
                                                    "Lengkapi Isian",
                                                    Toast.LENGTH_SHORT).show();
                                        } else {
                                            int status = stat.getCheckedRadioButtonId();
                                            int statusIsian = statI.getCheckedRadioButtonId();
                                            String s1 = null;
                                            if (status == R.id.returned) {
                                                s1 = InstanceProviderAPI.STATUS_RETURNED;
                                            } else {
                                                s1 = InstanceProviderAPI.STATUS_CLEARED;
                                            }
                                            String s2 = null;
                                            if (statusIsian == R.id.clear) {
                                                s2 = InstanceProviderAPI.STATUS_CLEAR;
                                            } else {
                                                s2 = InstanceProviderAPI.STATUS_UNCLEAR;
                                            }
                                            savekortim(s1, s2, instanceComplete
                                                    .isChecked(), saveAs.getText()
                                                    .toString());
                                        }
//									saveDataToDisk(EXIT, instanceComplete
...
//											.toString());
            ...
            case FormEntryController.EVENT_REPEAT:
                ...
                return odkv;
//			case EVENT_EDIT_ERROR:
//				ODKView odkva =null ;
//				// should only be a group here if the event_group is a field-list
//				try {
//					FormEntryPrompt[] prompts=new FormEntryPrompt[errorList.size()];
//					for (int x=0; x<errorList.size();x++) {
//						FormIndex oka= formController.getIndexFromXPath(errorList.get(x).getXPath());
//						prompts[x]=formController.getQuestionPrompt(oka);
//					}
//					FormEntryCaption[] groups = formController
//							.getGroupsForCurrentIndex();
//					odkva = new ODKView(this,	prompts,
//							groups, advancingPage);
//					Log.i("budi",
//							"created view for group "
//									+ (groups.length > 0 ? groups[groups.length - 1]
...
//
//				// Makes a "clear answer" menu pop up on long-click
//				for (QuestionWidget qw : odkva.getWidgets()) {
//					if (!qw.getPrompt().isReadOnly()) {
//						registerForContextMenu(qw);
//					}
//				}
...
//				return odkva;
    ...
    private void savekortim(String status, String statusisian, boolean check, String nama) {
        Log.d("savekortim1", status);
        Log.d("savekortim2", statusisian);
        FormController fc = Collect.getInstance().getFormController();
        fc.setMetadataTambahan(status, statusisian);
        ContentValues value = new ContentValues();
        value.put(InstanceColumns.STATUS_ISIAN, statusisian);
        value.put(InstanceColumns.PERLAKUAN, status);
        String instancePath = fc.getInstancePath().getAbsolutePath();
        ...
        int updated =
                Collect.getInstance().getContentResolver()
                        .update(InstanceColumns.CONTENT_URI, value, where, whereArgs);
        Log.d("savekortim4", "" + updated);
        saveDataToDisk(EXIT, check, nama, 1);
    }
...
//				.getFormController();
//		formController.validates(true);
        Intent i = new Intent(getApplicationContext(), FormHierarchyActivity.class);
        startActivity(i);
    }
    ...
    private void showNextViewWithoutConstraint() {
        try {
            FormController formController = Collect.getInstance()
                    .getFormController();
            ...
            for (FormEntryPrompt p : prompts) {
                ...
                for (int i = 0; i < attrs.size(); i++) {
                    if (!mAutoSaved && "saveIncomplete".equals(attrs.get(i).getName())) {
                        saveDataToDisk(false, false, null, false, 0);
                        mAutoSaved = true;
                    }
                }
            }
     ...
     */
    private void createConstraintToast(ArrayList<FailedConstraint> Failed) {
        FormController formController = Collect.getInstance()
                .getFormController();
        StringBuilder constraintText = new StringBuilder();
        for (int in = 0; in < Failed.size(); in++) {
...
//        showCustomToast(constraintText.toString(), Toast.LENGTH_SHORT);
        Toast t = Toast.makeText(this, constraintText.toString(), Toast.LENGTH_SHORT);
    ...
    private void showCustomToast(String message, int duration) {
        ...
        View view = inflater.inflate(R.layout.toast_view, null, false);
    ...
    private void createNoteDialog() {
        final FormController fc = Collect.getInstance().getFormController();
        if (fc.getCatatan() != null) {
            final Dialog dNote = new Dialog(this);
            dNote.setContentView(R.layout.note_dialog);
            dNote.setTitle("Tulis Catatan");

            final EditText catatanText = (EditText) dNote.findViewById(R.id.editText);
            catatanText.setText(fc.getCatatan().toString());
            catatanText.setFilters(new InputFilter[]{new InputFilter.AllCaps()});
            TextView simpanBut = (TextView) dNote.findViewById(R.id.simpanCat);
            TextView cancelBut = (TextView) dNote.findViewById(R.id.cancelCat);

            simpanBut.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    fc.setCatatan(catatanText.getText().toString());
                    Toast.makeText(FormEntryActivity.this, catatanText.getText().toString(), Toast.LENGTH_SHORT).show();
                    dNote.dismiss();
                }
            });
            ...
            Toast.makeText(this, "Tidak ada catatan dalam Kuesioner", Toast.LENGTH_LONG).show();
        }
    }
    ...
    private boolean saveDataToDisk(boolean exit, boolean complete, String updatedSaveName, int ak) {
        return saveDataToDisk(exit, complete, updatedSaveName, true, ak);
    }
    ...
    private boolean saveDataToDisk(boolean exit, boolean complete, String updatedSaveName,
        ...
        synchronized (saveDialogLock) {
            mSaveToDiskTask = new SaveToDiskTask(getIntent().getData(), exit, complete,
                    updatedSaveName, Collect.getInstance().getFormController().getError(), ak);
...
//	private boolean ForcesaveDataToDisk(boolean exit, boolean complete, String updatedSaveName) {
//		return ForcesaveDataToDisk(exit, complete, updatedSaveName, true);
//	}
...
//					updatedSaveName,Collect.getInstance().getFormController().getError());
//			mSaveToDiskTask.setFormSavedListener(this);
//			mAutoSaved = true;
//			showDialog(SAVING_DIALOG);
//			// show dialog before we execute...
...
//	}
    ...
    public void loadingComplete(FormLoaderTask task) {
        ...
        if (formController.getInstancePath() == null) {
            ...
            if(file.contains("pod")){
                path = Collect.INSTANCES_PATH + File.separator +  file + "_"
                        + pref.get(CapiKey.KEY_NIM_KORTIM)+"_"+timeStamp;
            }
            Log.e("SANDY", path);
            if (FileUtils.createFolder(path)) {
                if(file.contains("pod")){
                    formController.setInstancePath(new File(path + File.separator
                            + file + "_" + pref.get(CapiKey.KEY_NIM_KORTIM) +"_"+timeStamp+ ".xml"));
                }else{
                    formController.setInstancePath(new File(path + File.separator
                            + file + "_" + time + ".xml"));
                }
        ...
        formController.setBeforeMetadata();
    ...
    public void savingComplete(SaveResult saveResult) {
        ...
        Log.d(t, "saving complete " + saveStatus);
        switch (saveStatus) {
